 ...
 *     ticket files on disk via `parseTicketFrontmatter`; unresolved => NULL
...
function projectKey(projectSlug: string, ticketSlug: string): string {
...
function buildSlugIdMaps(): {
  ...
  for (const proj of safeReaddir(projectsDir)) {
    ...
    const asgRoot = resolve(projectsDir, proj.name, 'tickets');
    for (const asg of safeReaddir(asgRoot)) {
      if (!asg.isDirectory()) continue;
      const id = readTicketId(resolve(asgRoot, asg.name, 'ticket.md'));

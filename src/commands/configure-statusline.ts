...
export type SegmentName = 'wrap' | 'git' | 'ticket' | 'external' | 'session' | 'model' | 'ctx' | 'cwd';
...
export const PRESETS: Record<string, { segments: SegmentName[]; separator: string }> = {
  ...
  syntaur: { segments: ['git', 'ticket', 'session'], separator: ' · ' },
  full:    { segments: ['wrap', 'git', 'ticket', 'external', 'model', 'ctx', 'session'], separator: ' · ' },
  dev:     { segments: ['git', 'ticket', 'external', 'ctx', 'session'], separator: ' · ' },
  tracker: { segments: ['git', 'ticket', 'external', 'session'], separator: ' · ' },
};
...
async function promptSegmentsInteractive(
  ...
  const canonicalOrder: SegmentName[] = ['wrap', 'git', 'ticket', 'external', 'session', 'model', 'ctx', 'cwd'];

  const selectedSet = new Set(current?.segments ?? ['git', 'ticket', 'session']);

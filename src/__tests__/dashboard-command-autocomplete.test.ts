...
describe('rankCommands', () => {
  const commands: ChatCommand[] = [
    { name: 'plan-assignment', description: 'Plan work', inputHint: null, action: { kind: 'prompt' } },
  ...
  it('ranks prefix matches first and caps at eight', () => {
    expect(rankCommands('plan', commands).map((c) => c.name)).toEqual(['plan', 'plan-assignment', 'status']);
  });
});
...
describe('applyCommand', () => {
  it('inserts /name with a trailing space', () => {
    ...
    expect(applyCommand('/pla', active, 'plan-assignment')).toEqual({

...
describe('custom facts + attestations CLI (end-to-end)', () => {
  ...
  it('AC7 forward + reverse: fact set + attest fire the rung; replan regresses it', async () => {
    expect((await runCli(['recompute', 'FTX-1', '--project', 'p1'], home)).code).toBe(0);
    expect((await fm()).status).toBe('ready_for_planning');

    await writeFile(join(aDir, 'plan.md'), '# Plan v1');
    await runCli(['plan', 'approve', 'FTX-1', '--project', 'p1'], home);
    await runCli(['implement', 'FTX-1', '--project', 'p1'], home);
    expect((await fm()).status).toBe('in_progress');

    // custom bool fact alone is not enough (codeReview not approved)
    let r = await runCli(['fact', 'set', 'FTX-1', 'qaPassed', 'true', '--project', 'p1'], home);
    expect(r.code).toBe(0);
    expect((await fm()).status).toBe('in_progress');
    expect((await fm()).facts.qaPassed).toBe('true');

    // attest the plan-bound review → shipped rung fires
    r = await runCli(['attest', 'FTX-1', 'codeReview', '--verdict', 'approved', '--agent', 'codex', '--project', 'p1'], home);
    expect(r.code).toBe(0);
  ...
  it('re-attest replaces the actor record; default verdict is approved', async () => {
    await runCli(['recompute', 'FTX-1', '--project', 'p1'], home);
    await writeFile(join(aDir, 'plan.md'), '# Plan');
    await runCli(['attest', 'FTX-1', 'codeReview', '--agent', 'codex', '--verdict', 'changes-requested', '--project', 'p1'], home);
    await runCli(['attest', 'FTX-1', 'codeReview', '--agent', 'codex', '--project', 'p1'], home); // default approved, replaces
  ...
  it('binds:commit attestation goes stale after a new commit', async () => {
    ...
    try {
      await runCli(['recompute', 'FTX-1', '--project', 'p1'], home);
      let r = await runCli(['attest', 'FTX-1', 'deploy', '--agent', 'ci', '--project', 'p1'], home);
      expect(r.code).toBe(0);
  ...
  it('binds:commit attest refuses when the workspace is not a git repo', async () => {
    ...
    await runCli(['recompute', 'FTX-1', '--project', 'p1'], home);
    const r = await runCli(['attest', 'FTX-1', 'deploy', '--agent', 'ci', '--project', 'p1'], home);
  ...
  it('errors: undeclared fact, type mismatch, non-attestation attest, bad verdict', async () => {
    ...
    let r = await runCli(['fact', 'set', 'FTX-1', 'bogus', 'true', '--project', 'p1'], home);
    expect(r.code).not.toBe(0);
    expect(r.stderr).toMatch(/not a declared custom fact/i);

    r = await runCli(['fact', 'set', 'FTX-1', 'qaPassed', 'notabool', '--project', 'p1'], home);
    expect(r.code).not.toBe(0);
    expect(r.stderr).toMatch(/not a valid bool/i);

    r = await runCli(['attest', 'FTX-1', 'qaPassed', '--project', 'p1'], home); // qaPassed is bool, not attestation
    expect(r.code).not.toBe(0);
    expect(r.stderr).toMatch(/not a declared attestation/i);

    await writeFile(join(aDir, 'plan.md'), '# Plan');
    r = await runCli(['attest', 'FTX-1', 'codeReview', '--verdict', 'maybe', '--project', 'p1'], home);
    expect(r.code).not.toBe(0);
  ...
  it('verbs run against a PRE-migration home (explicit verbs are not marker-gated)', async () => {
    ...
    const r = await runCli(['fact', 'set', 'FTX-1', 'qaPassed', 'true', '--project', 'p1'], home);
    expect(r.code).toBe(0);
    expect((await fm()).facts.qaPassed).toBe('true');
  });
  ...
  it('AC9: a dimension-stable fact set appends a same-status audit entry + bumps updated', async () => {
    ...
    const r = await runCli(['fact', 'set', 'FTX-1', 'storyPoints', '5', '--project', 'p1'], home);
    expect(r.code).toBe(0);
    ...
    await runCli(['fact', 'set', 'FTX-1', 'storyPoints', '5', '--project', 'p1'], home);

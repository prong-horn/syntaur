 ...
 *
 * Takes the SAME configured `projectsDir` the server holds so
 ...
 */
export function createContentSearchRouter(projectsDir: string): Router {
  const router = Router();
  ...
  router.get('/', async (req, res) => {
    try {
      ...
      const docs = await getIndex({ projectsDir, includeArchived });

...
description: >-
  Manage custom ticket statuses and transitions in Syntaur. Use when the
...
Customize Syntaur's ticket-status workflow via the `syntaur status` CLI. The CLI writes to the same `statuses:` block in `~/.syntaur/config.md` that the dashboard Settings page edits — so CLI and dashboard edits stay in sync.
...
| `syntaur status remove <id> [--force]` | Remove a status. Without `--force`, errors if any ticket references it; lists offenders. With `--force`, also prunes `order` and `transitions[i]` whose `from === <id>` or `to === <id>`. |
| `syntaur status rename <id> --to <new-id> [--label <label>]` | Rename a status id. **Atomic** rewrite across `config.md` + every affected `ticket.md`. Without `--label`, keeps the original label. |
...
Every mutating subcommand supports `--dry-run`, which prints a unified diff of the would-be `statuses:` block change (and, for `rename`, a per-file frontmatter diff for each affected `ticket.md`) and exits without writing.
...
- **After a `rename`,** every ticket that referenced the old id has been rewritten in-place. Mention this to the user — git will show diffs in many `ticket.md` files. They are intentional.
- **After a `remove --force`,** the affected tickets now reference an undefined status. `syntaur doctor` will flag them as invalid. Suggest the user run `syntaur doctor` and either re-add the status (`syntaur status add ...`) or edit each frontmatter to a valid id.
...
```bash
syntaur fact set <ticket> qaPassed true --project <p>
syntaur fact set <ticket> storyPoints 5 --project <p>
```
...
```bash
syntaur attest <ticket> codeReview --agent codex --verdict approved --project <p>
syntaur attest <ticket> codeReview --agent pi --verdict changes-requested --note "fix the lock" --project <p>
```
...
- **`remove` is destructive.** Without `--force` it refuses if any ticket references the id. Don't suggest `--force` without first running `syntaur status remove <id>` (no force) so the user sees the affected list.
...
- **`terminal: true` is load-bearing.** Terminal statuses affect dashboard progress bars and dependency-satisfaction logic — an ticket with a terminal status counts as "done" for downstream `dependsOn` checks and project-rollup status. Don't toggle this on `pending`-style states without thinking.

...
- **`terminal: true` is load-bearing.** Terminal statuses affect dashboard progress bars and dependency-satisfaction logic — a ticket with a terminal status counts as "done" for downstream `depends_on` checks and project-rollup status. Don't toggle this on `pending`-style states without thinking.

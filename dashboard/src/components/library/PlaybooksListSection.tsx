...
import { deletePlaybook, setPlaybookEnabled } from '../../data/libraryResources';
...
export interface PlaybooksListSectionProps {
  linkPrefix?: string;
}

export function PlaybooksListSection({ linkPrefix = '/library/playbooks' }: PlaybooksListSectionProps) {
  ...
  async function handleToggle(event: React.MouseEvent<HTMLButtonElement>, slug: string, enabled: boolean) {
    ...
    setActionError(null);
    try {
      await setPlaybookEnabled(slug, !enabled);
      refetch();
    } catch (err) {
  ...
  async function handleConfirmDelete() {
    ...
    setActionError(null);
    try {
      await deletePlaybook(deleteTarget.slug);
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-xs text-muted-foreground">
            Index of all playbooks. Include{' '}
            <code className="rounded bg-muted px-1 py-0.5 text-[11px]">~/.syntaur/playbooks/manifest.md</code> in agent context.
          </p>
        </div>
      </Link>
          ...
          {filtered.map((playbook) => {
            ...
            return (
              ...
              >
                ...
                <Link to={`${base}/${playbook.slug}`} className="group flex flex-col gap-2">
                  <div className="flex items-start gap-2">
                    <BookOpen className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
                    <div className="min-w-0 flex-1">
                      <h3 className="text-sm font-semibold text-foreground group-hover:underline">{playbook.name}</h3>
                      {playbook.description ? (
                        <p className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">{playbook.description}</p>
                      ) : null}
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-1 pr-20 pt-1">
                ...
                <button
                  ...
                  disabled={isPending}
                  className={`absolute bottom-3 right-3 z-10 inline-flex h-5 w-9 items-center rounded-full border transition disabled:opacity-50 ${
                    playbook.enabled
      ...
      <ConfirmDialog
        ...
        description={
          deleteTarget
            ? `"${deleteTarget.name}" will be removed from disk and the manifest regenerated. This cannot be undone.`
            : ''
        }
        confirmLabel="Delete"
        destructive

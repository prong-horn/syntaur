...
import { AgentListRow } from '../agents/AgentListRow';
...
export interface AgentsListSectionProps {
  linkPrefix?: string;
}

export function AgentsListSection({ linkPrefix = '/library/agents' }: AgentsListSectionProps) {
  ...
  const [testById, setTestById] = useState<Record<string, AgentTestResult | 'loading'>>({});
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Definitions live in <code className="font-mono">~/.syntaur/agents/&lt;id&gt;.md</code>.
          </p>
        </div>
        <Link to={`${base}/new`} className="shell-action shell-action--cta">
      ...
      {agents.length === 0 ? (
        <EmptyState title="No agents yet" description="Create one above, or add a file under ~/.syntaur/agents/." />
      ) : (
        <div className="grid gap-2">
          {agents.map((agent) => (
            <AgentListRow
              ...
              onEdit={() => navigate(`${base}/${agent.id}/edit`)}

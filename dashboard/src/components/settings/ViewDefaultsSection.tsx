...
export function ViewDefaultsSection() {
  ...
  return (
    <SectionCard
      title="View defaults"
      description="Defaults for the board kanban, list, and table views. Per-project overrides are saved automatically as you use them."
      actions={
        file.custom ? (
          <button type="button" className="shell-action text-xs" onClick={handleReset} disabled={saving}>
    ...
    >
      ...
      <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          <dt className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Default view</dt>
            ...
            {VIEW_MODES.map((v) => (
              <button
                ...
                className={`px-3 py-1 text-xs ${
                  global.defaultView === v ? 'rounded bg-foreground text-background' : 'text-muted-foreground hover:text-foreground'
                }`}
              >
                {VIEW_LABEL[v]}
              </button>
        ...
        <div>
          <dt className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Density</dt>
          <dd className="mt-1.5 inline-flex rounded-md border border-border/60 p-0.5">
            {DENSITIES.map((d) => (
              <button
                key={d}
                ...
                className={`px-3 py-1 text-xs ${
                  global.density === d ? 'rounded bg-foreground text-background' : 'text-muted-foreground hover:text-foreground'
                }`}
              >
                {DENSITY_LABEL[d]}
              </button>
        ...
        <div>
          <dt className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Sort by</dt>
          <dd className="mt-1.5">
            <select
          ...
          <dd className="mt-1.5 inline-flex rounded-md border border-border/60 p-0.5">
            {SORT_DIRECTIONS.map((d) => (
              <button
                ...
                className={`px-3 py-1 text-xs ${
                  global.sortDirection === d ? 'rounded bg-foreground text-background' : 'text-muted-foreground hover:text-foreground'
                }`}
              >
                {DIRECTION_LABEL[d]}
              </button>

...
export function SearchSection() {
  ...
  return (
    <SectionCard
      title="Search"
      description="Default scope, type-alias prefixes, and external-ID indexing for the search dialog."
      actions={
        custom ? (
          <button
    ...
    >
      ...
      <div className="space-y-5">
        <label className="block text-sm">
          <span className="mb-1.5 block text-xs font-medium text-muted-foreground">Default scope</span>
          <select
            value={defaultScope}
            onChange={(e) => {
        ...
        <div className="text-sm">
          <span className="mb-1.5 block text-xs font-medium text-muted-foreground">Type-alias prefixes</span>
              ...
              <div key={i} className="flex items-center gap-2">
                ...
                >
                  {ENTITY_KINDS.map((kind) => (
                    <option key={kind} value={kind}>{SCOPE_LABELS[kind]}</option>
                  ))}
                </select>
                <button type="button" onClick={() => removeRow(i)} disabled={saving} aria-label={`Remove alias ${row.prefix}`}>
                  <X className="h-3.5 w-3.5" />
                </button>
              </div>
            ))}
          </div>
          <button type="button" onClick={addRow} disabled={saving} className="mt-2 inline-flex items-center gap-1 text-xs text-primary hover:underline">
        ...
        <label className="flex items-start gap-2 text-sm">
          ...
          <span>
            ...
            <span className="mt-0.5 block text-xs text-muted-foreground">
              Fold external IDs into search and enable <code>jira:</code>/<code>externalid:</code> matching.
            </span>
          </span>
        </label>

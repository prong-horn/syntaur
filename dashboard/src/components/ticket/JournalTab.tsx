...
import { useResource, useResourceStore } from '../../data/useResource';
import {
  appendTicketLogEntry,
  ticketResources,
  type JournalAppendPayload,
} from '../../data/ticketResources';
...
export function JournalTab({ ticketId, file, onAppended }: JournalTabProps) {
  ...
  const allLog = useResource(ticketResources.log(ticketId));
  const filteredLog = useResource(typeFilter ? ticketResources.log(ticketId, typeFilter) : null);

  const allEntries = allLog.data?.entries ?? file.logEntries ?? [];
  const displayEntries = typeFilter
    ? (filteredLog.data?.entries ?? [])
    : allEntries;
  const loading = typeFilter ? filteredLog.loading || allLog.loading : allLog.loading;
  const error =
    actionError ??
    (typeFilter ? filteredLog.error?.message ?? allLog.error?.message : allLog.error?.message) ??
    null;
  ...
  const reload = useCallback(async () => {
    await Promise.all([allLog.refetch(), typeFilter ? filteredLog.refetch() : Promise.resolve()]);
  }, [allLog, filteredLog, typeFilter]);
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    setActionError(null);
    try {
      await appendTicketLogEntry(ticketId, payload, { store });
      setBody('');
      setAnswers('');
      await reload();
      onAppended?.();
    } catch (err) {
      setActionError(err instanceof Error ? err.message : 'Failed to append entry');
    } finally {
      setSubmitting(false);
    }
  }
        ...
        {displayEntries.length === 0 && !loading ? (
          ...
          <ol className="space-y-4">
            {displayEntries.map((entry, idx) => {
              const verdictBadge =
                entry.type === 'review' ? parseVerdictBadge(entry.keys?.verdict) : null;
              const attachmentNames =
                entry.keys?.attachments
                  ?.split(',')
                  .map((s) => s.trim())
                  .filter(Boolean) ?? [];
              return (

...
/**
 * These tests drive the public ticket editor entry points so the YAML
...
function ticketDoc(frontmatter: string): string {
...
describe('documents YAML scalar codec + inline-flow lists', () => {
  it('(a) inline `tags: [a, b]` survives parse -> serialize -> parse (B1)', () => {
    ...
    const serialized = updateTicketContent(content, { body: 'Changed.' });
    const reparsed = parseTicketEditorState(serialized);
  ...
  it('(b) literal `"quoted"` scalar round-trips (B6)', () => {
    ...
    const serialized = updateTicketContent(ticketDoc('title: x'), {
      title: '"quoted"',
    });
  ...
  it('(e) empty `[]` stays empty', () => {
    ...
    expect(parseTicketEditorState(content).tags).toBe('');

    const serialized = updateTicketContent(content, { tags: '' });
    expect(serialized).toContain('tags: []');
    expect(parseTicketEditorState(serialized).tags).toBe('');
  });
  ...
  it('(f) a list element containing a comma and/or quote round-trips', () => {
    ...
    const inline = ticketDoc(
      'title: Demo\nstatus: x\ntags: ["a, b", "she said \\"hi\\""]',
    );

    const parsed = parseTicketEditorState(inline);

...
export function CreateMemory() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      const response = await fetch(`/api/projects/${projectSlug}/memories`, {
        method: 'POST',
      ...
      navigate(`/projects/${payload.projectSlug}/memories/${payload.slug}`);
    } catch (saveError) {
  ...
  if (!projects || projects.length === 0) {
    return (
      <ErrorState error="You need at least one project before you can create a memory. Create a project from the sidebar." />
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div>
        <h2 className="text-lg font-semibold text-foreground">New Memory</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          Memories capture patterns and learnings discovered during work in a project. They are
          available to every assignment in that project.
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-1.5">
          ...
          <input
            ...
            placeholder="e.g. PostgreSQL connection pooling"
            className="editor-input"
            required
            autoFocus
          />
      ...
      <div className="space-y-1.5">
        ...
        <textarea
          ...
          rows={14}
          placeholder="Write the memory body in markdown. Leave blank to start from a template."

...
1. When planning an assignment for the first time, write `plan.md` under the assignment directory (via `syntaur plan create` or the `plan-assignment` skill).
...
2. If asked to create a new plan after a prior plan has been implemented, write the next versioned file (`plan-v2.md`, `plan-v3.md`, …) via `syntaur plan version` or the `replan` skill.
   ...
   Leave prior plan files intact on disk — the history of plan cycles is part of the assignment record.

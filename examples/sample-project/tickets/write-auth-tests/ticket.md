...
This ticket depends on [implement-jwt-middleware](../implement-jwt-middleware/ticket.md) being completed. Tests will cover both the schema layer (from design-auth-schema) and the middleware/endpoint layer (from implement-jwt-middleware). Use Jest as the test framework with `supertest` for HTTP integration tests.

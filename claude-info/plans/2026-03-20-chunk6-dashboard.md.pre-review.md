...
|------|----------------|---------------|
| `src/index.ts` | CLI entry with 12 commands (L1-L226) | Add `dashboard` command import and registration |
...
1. [ ] **Step 1.1:** Add runtime dependencies for the dashboard server
   ...
   - **Action:** MODIFY
   - **What to do:** Add `express`, `chokidar`, `ws`, and `open` to the `dependencies` object. Add `@types/express` and `@types/ws` to `devDependencies`.
   - **Code:**
     ```json
     "dependencies": {
       "chokidar": "^4.0.0",
...
- [ ] `package.json` has `@types/express`, `@types/ws` in `devDependencies`
   ...
   - **Code:**
     ...
     export function createDashboardServer(options: DashboardServerOptions) {
       ...
       if (devMode) {
         ...
         // Look for dashboard/dist relative to where the compiled JS lives
         // When installed via npm, the structure is: dist/dashboard/server.js and dashboard/dist/
         // When running locally, it's: dist/dashboard/server.js -> ../../dashboard/dist/
...
**Note on dev mode proxy:** The `http-proxy-middleware` package is used for dev mode proxy. This needs to be added as an **optional** dev dependency. However, for simplicity, we will use a simpler approach -- skip the proxy in dev mode and have the Vite dev server handle both API proxy (via its own config) and SPA serving. The server just starts the API on the given port, and Vite proxies API calls to it. Revise Step 6.1 to remove the dev proxy logic:

Replace the dev proxy section with:
```typescript
     if (!devMode) {
       // Serve the built dashboard SPA
       const dashboardDistPath = resolve(__dirname, '..', '..', 'dashboard', 'dist');
...
This means in dev mode, the Express server only serves the API and WebSocket. Vite dev server (on port 5173) serves the SPA and proxies `/api` and `/ws` to the Express server. This avoids needing `http-proxy-middleware` as a dependency.
...
1. [ ] **Step 7.1:** Create `src/commands/dashboard.ts`
   ...
   - **Code:**
     ...
     ```
   - **Proof blocks:**
     - **PROOF:** `readConfig()` returns `SyntaurConfig` with `defaultMissionDir` -- Source: `src/utils/config.ts:51-86`
       ```typescript
...
1. [ ] **Step 14.1:** Add `src/dashboard/server.ts` as a second entry point in tsup
   ...
   - **Action:** MODIFY
   - **What to do:** Change the `entry` array from a single entry to two entries. Also enable `splitting` so the two entry points can share common code.
   - **Code:**
     ```typescript

...
| `src/templates/index.ts` | Barrel export for all templates (L1-L40) | MODIFY: Add new template exports |
...
| Template barrel export | `/Users/brennen/syntaur/src/templates/index.ts` | L1-L40 | Pattern of `export { renderX } from './x.js'; export type { XParams } from './x.js';` for each template |
...
**Action:** MODIFY
**Pattern Reference:** `src/templates/index.ts:1-40` (existing barrel export pattern)
**Estimated complexity:** Low
...
1. [ ] **Step 4.1:** Append export lines for the three new template modules at the end of `src/templates/index.ts`.
   - **Location:** `src/templates/index.ts:40` (after the last existing export line)
   - **Action:** MODIFY
...
### Task 7b: Create CLI Integration Tests for setup-adapter

**File(s):** `src/__tests__/setup-adapter.test.ts`
**Action:** CREATE
**Pattern Reference:** `src/__tests__/lifecycle-commands.test.ts` (integration test pattern with fs setup/teardown)
**Estimated complexity:** Medium

#### Context
Task 7 covers unit tests for the template renderers. This task adds integration tests for the `setup-adapter` CLI command itself, verifying end-to-end behavior: file generation for each framework, skip-on-exists behavior, and `--force` override.

#### Steps

1. [ ] **Step 7b.1:** Create the integration test file `src/__tests__/setup-adapter.test.ts`.
   - **Location:** new file at `src/__tests__/setup-adapter.test.ts`
   - **Action:** CREATE
   - **What to do:** Import `setupAdapterCommand` from the command module. Create a temp directory with a minimal mission/assignment structure (mission.md, assignment.md) for each test. Test the following scenarios:
     - `syntaur setup-adapter cursor` generates `.cursor/rules/syntaur-protocol.mdc` and `.cursor/rules/syntaur-assignment.mdc`
     - `syntaur setup-adapter codex` generates `AGENTS.md`
     - `syntaur setup-adapter opencode` generates `AGENTS.md` and `opencode.json`
     - Skip-on-exists: when target files already exist, they are not overwritten (no `--force`)
     - `--force` override: when target files already exist and `--force` is set, they are overwritten
     - Invalid framework name throws an error
     - Missing mission throws an error
     - Missing assignment throws an error
   - **Code:**
     ```typescript
     ...
     ```
   - **Proof blocks:**
     - **PROOF:** Integration test pattern with fs setup/teardown from `src/__tests__/lifecycle-commands.test.ts` uses `mkdtemp`, `rm`, `beforeEach`/`afterEach` for temp directory management.
     - **PROOF:** `setupAdapterCommand` accepts `(framework: string, options: SetupAdapterOptions)` as defined in Task 5.
     - **PROOF:** `writeFileSafe` returns false when file exists (skip behavior) from `src/utils/fs.ts:17-27`. `writeFileForce` always overwrites from `src/utils/fs.ts:29-35`.
   - **Verification:**
     ```bash
     cd /Users/brennen/syntaur && npx vitest run src/__tests__/setup-adapter.test.ts
     ```
...
#### Task Completion Criteria
- [ ] File `src/__tests__/setup-adapter.test.ts` exists
- [ ] Tests cover: cursor file generation, codex file generation, opencode file generation, skip-on-exists, --force override, invalid framework, missing mission, missing assignment
- [ ] All tests pass with `npx vitest run src/__tests__/setup-adapter.test.ts`
...
#### Context
Static files in the `adapters/` directory are **documentation and examples**, not runtime artifacts. The TypeScript renderers in `src/templates/` embed all protocol content directly in template literals -- they never read from these files. The `adapters/` files exist so that contributors can see the expected format for each framework without reading TypeScript source, and to serve as a reference when adding support for new frameworks. Each file is a short stub pointing users to the `syntaur setup-adapter` command for actual generation.
...
1. [ ] **Step 8.1:** Create `adapters/cursor/syntaur-protocol.mdc` as a reference example of the Cursor .mdc format.
   ...
   - **Action:** CREATE
   - **What to do:** Write a documentation file showing the Cursor .mdc format with a stub pointing to the CLI command. This is documentation, not a runtime artifact -- the renderer embeds content directly.
   - **Code:**
     ```
     ---
     description: Syntaur protocol rules for multi-agent coordination
...
2. [ ] **Step 8.2:** Create `adapters/codex/AGENTS.md.template` as a reference example.
   ...
   - **Action:** CREATE
   - **What to do:** Write a documentation file showing the AGENTS.md format with a stub pointing to the CLI command. This is documentation, not a runtime artifact.
   - **Code:**
     ```markdown
...
3. [ ] **Step 8.3:** Create `adapters/opencode/opencode.json.template` as a reference example.
   ...
   - **Action:** CREATE
   - **What to do:** Write a documentation file showing the `opencode.json` structure with placeholder values. This is documentation, not a runtime artifact.
   - **Code:**
     ```json
     {
       "instructions": [
...
#### Task Completion Criteria
- [ ] All three documentation files exist in the `adapters/` directory
- [ ] Each file clearly states it is documentation/example content, not a runtime artifact
- [ ] Each file points users to the `syntaur setup-adapter` command for actual generation
...
7b. **Task 7b** -- CLI integration tests (depends on Tasks 5-6)
8. **Task 8** -- Static adapter documentation files (no code dependencies, can be done anytime)
...
- **Total tasks:** 10
- **Total steps:** 13
- **Files to create:** 10 (`src/templates/cursor-rules.ts`, `src/templates/codex-agents.ts`, `src/templates/opencode-config.ts`, `src/commands/setup-adapter.ts`, `src/__tests__/adapter-templates.test.ts`, `src/__tests__/setup-adapter.test.ts`, `adapters/cursor/syntaur-protocol.mdc`, `adapters/codex/AGENTS.md.template`, `adapters/opencode/opencode.json.template`, `adapters/README.md`)
...
| Task 4, Step 4.1 | Yes | Yes (L40) | Yes, full code | Yes, 5 proofs | PASS |
...
### Revision 2026-03-21 (Codex review feedback)

**Pre-revision snapshot:** `claude-info/plans/2026-03-21-chunk7-additional-adapters.md.pre-revision-2026-03-21.md`

| # | Issue | Resolution |
|---|-------|------------|
| 1 | Stale line numbers: plan said `src/templates/index.ts` has 41 lines, actual is 40 | Fixed all references from `L1-L41` to `L1-L40` and `index.ts:41` to `index.ts:40` in Tasks 4, Patterns table, and Files table |
| 2 | Unused `writeFn` in Task 5 code: defined but never used, code used inline `writeFileForce`/`writeFileSafe` instead | Removed `writeFn` variable. Extracted `writeAdapterFile()` helper that encapsulates the force/safe logic, simplifying all framework branches |
| 3 | No CLI integration tests: Task 7 only covered renderer unit tests | Added Task 7b with `src/__tests__/setup-adapter.test.ts` covering: cursor/codex/opencode generation, skip-on-exists, --force override, invalid framework, missing mission, missing assignment |
| 4 | Missing transition table in adapter content: adapters had lifecycle states and CLI commands but not the valid-transitions table from protocol-summary.md | Added "Valid State Transitions" table (10 rows matching `src/lifecycle/state-machine.ts` TRANSITION_TABLE) to both Cursor protocol renderer (Task 1) and Codex AGENTS.md renderer (Task 2) |
| 5 | Task 8 static templates purpose unclear: described as "should match what renderers produce" but actually creates documentation stubs | Rewrote Task 8 context and step descriptions to explicitly state these are documentation files and format examples, not runtime artifacts |
| 6 | Workspace boundary handling ambiguous: adapters referenced "workspace defined in assignment frontmatter" but setup-adapter never reads assignment frontmatter | Chose simpler approach: adapters use generic guidance ("current working directory is your workspace; read assignment frontmatter's `workspace` field at runtime for exact boundary"). Added explicit notes to both Cursor and Codex adapter content clarifying that workspace resolution happens at agent runtime, not at adapter generation time |

**Counts:** Total tasks 9 -> 10. Total steps 12 -> 13. Files to create 9 -> 10.

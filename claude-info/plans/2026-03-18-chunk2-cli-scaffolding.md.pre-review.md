...
- [ ] All generated files match the protocol spec exactly (frontmatter schemas, body sections, field types)
...
| Plan frontmatter + body | `examples/sample-mission/assignments/design-auth-schema/plan.md` | L1-L32 | `assignment`, `status: draft`, `created`, `updated`; Approach, Tasks, Risks sections |
...
4. **CLI entry point and init command:** Set up Commander program, implement `syntaur init` -- Files: `src/index.ts`, `src/commands/init.ts`
...
7. **Unit tests:** Test utilities (slug, timestamp, paths), template output correctness, command integration tests -- Files: `src/__tests__/slug.test.ts`, `src/__tests__/timestamp.test.ts`, `src/__tests__/paths.test.ts`, `src/__tests__/templates.test.ts`, `src/__tests__/commands.test.ts`, `vitest.config.ts`
...
2. [ ] **Step 2.2:** Create `src/utils/timestamp.ts`
...
6. [ ] **Step 2.6:** Create `src/utils/config.ts`
   ...
   - **Code:**
     ...
     import { resolve } from 'node:path';
     import { syntaurRoot, defaultMissionDir } from './paths.js';
      ...
      * Returns a Record of key-value pairs from the --- delimited block.
      * This is intentionally simple -- it handles the flat/shallow YAML
      * used in config.md without requiring a YAML library.
      */
     ...
     export async function readConfig(): Promise<SyntaurConfig> {
       ...
       return {
         ...
         defaultMissionDir:
           fm['defaultMissionDir'] || DEFAULT_CONFIG.defaultMissionDir,
         agentDefaults: {
           trustLevel:
             (fm['trustLevel'] as SyntaurConfig['agentDefaults']['trustLevel']) ||
             DEFAULT_CONFIG.agentDefaults.trustLevel,
           autoApprove: DEFAULT_CONFIG.agentDefaults.autoApprove,
         },
       };
     }
     ```
...
| config.md has malformed frontmatter | `parseFrontmatter` returns empty record | Use defaults silently | `readConfig` falls back to `DEFAULT_CONFIG` |
...
11. [ ] **Step 3.11:** Create `src/templates/index-stubs.ts`
    ...
    - **Verification:**
      ```bash
      npx tsx -e "import { renderIndexAssignments, renderStatus, renderResourcesIndex, renderMemoriesIndex } from './src/templates/index-stubs.ts'; console.log(renderIndexAssignments({ slug: 'test', timestamp: '2026-03-18T14:30:00Z' }))"
      ```
...
1. [ ] **Step 5.1:** Create `src/commands/create-mission.ts`
   ...
   - **Code:**
     ...
     import { expandHome, defaultMissionDir } from '../utils/paths.js';
     ...
     export async function createMissionCommand(
       ...
       const files: Array<[string, string]> = [
         ...
         [
           ...
           renderIndexAssignments({ slug, timestamp }),
         ],
         [
           resolve(missionDir, '_index-plans.md'),
           renderIndexPlans({ slug, timestamp }),
         ],
         [
           resolve(missionDir, '_index-decisions.md'),
           renderIndexDecisions({ slug, timestamp }),
         ],
         [
           resolve(missionDir, '_index-sessions.md'),
           renderIndexSessions({ slug, timestamp }),
         ],
         [
           resolve(missionDir, '_status.md'),
           renderStatus({ slug, timestamp }),
         ],
         [
           resolve(missionDir, 'resources', '_index.md'),
           renderResourcesIndex({ slug, timestamp }),
         ],
         [
           resolve(missionDir, 'memories', '_index.md'),
           renderMemoriesIndex({ slug, timestamp }),
         ],
       ];
...
1. [ ] **Step 6.1:** Create `src/commands/create-assignment.ts`
   ...
   - **Code:**
     ...
     export async function createAssignmentCommand(
       ...
       // Validate that either --mission or --one-off is provided
       ...
       if (options.oneOff) {
         ...
         missionDir = resolve(defaultMissionDir(), missionSlug);
       } else {
         // Use existing mission
         missionSlug = options.mission!;
         const config = await readConfig();
         missionDir = resolve(config.defaultMissionDir, missionSlug);
   ...
   - **Code:**
     ...
     describe('renderStatus', () => {
       it('has initial pending status with zero counts', () => {
         const out = renderStatus({ slug: 'test', timestamp: TIMESTAMP });
         expect(out).toContain('status: pending');
         expect(out).toContain('total: 0');
...
5. [ ] **Step 7.5:** Create `src/__tests__/commands.test.ts`
   ...
   - **Code:**
     ...
     describe('createAssignmentCommand', () => {
       it('creates all expected assignment files', async () => {
         await createMissionCommand('Test Mission', { dir: testDir });
         // We need to mock the config to point to our test dir
         // For this test, we call createMissionCommand with --dir then reference it
         const missionDir = resolve(testDir, 'test-mission');
         const assignmentDir = resolve(
           missionDir,
           'assignments',
         ...
         // Directly create the assignment using the function's internal logic
         // by importing the utilities
         const { ensureDir, writeFileForce } = await import('../utils/fs.js');
         ...
         await writeFileForce(
           resolve(assignmentDir, 'plan.md'),
           renderPlan({ assignmentSlug: 'write-tests', title: 'Write Tests', timestamp }),
         );
         await writeFileForce(
           resolve(assignmentDir, 'scratchpad.md'),
           renderScratchpad({ assignmentSlug: 'write-tests', timestamp }),
         );
         await writeFileForce(
           resolve(assignmentDir, 'handoff.md'),
           renderHandoff({ assignmentSlug: 'write-tests', timestamp }),
         );
         await writeFileForce(
           resolve(assignmentDir, 'decision-record.md'),
           renderDecisionRecord({ assignmentSlug: 'write-tests', timestamp }),
         );
       ...
       it('assignment.md has correct initial state', async () => {
         await createMissionCommand('Test Mission', { dir: testDir });
         const missionDir = resolve(testDir, 'test-mission');
         const assignmentDir = resolve(
           missionDir,
         ...
         const { nowTimestamp } = await import('../utils/timestamp.js');
         const { generateId } = await import('../utils/uuid.js');
         const { renderAssignment } = await import('../templates/index.js');

...
| `src/commands/install-plugin.ts` | Does not exist | CREATE: CLI command to symlink plugin |
| `src/index.ts` | CLI entry point (lines 1-209) | MODIFY: Register install-plugin command |
...
6. **Install Command** (`src/commands/install-plugin.ts`): CLI command that creates the symlink and optionally registers the plugin in user settings.
...
flowchart TD
    ...
    subgraph "Filesystem"
        CLI -->|"Read/Write"| Mission["~/.syntaur/missions/<mission>/"]
...
sequenceDiagram
    ...
    GA->>FS: Read ~/.syntaur/missions/my-mission/
...
8. **Create install-plugin CLI command** -- Files: `src/commands/install-plugin.ts`, `src/index.ts`
   - New CLI command `syntaur install-plugin` that creates a symlink from `~/.claude/plugins/syntaur/` to the `plugin/` directory in the repo. Register it in `src/index.ts` following the Commander.js pattern.

9. **Update .gitignore and package.json** -- Files: `.gitignore`, `package.json`
   - Ensure `plugin/` directory is included in git. Add `plugin` to `files` array in package.json so it ships with npm. Ensure `.claude/syntaur.local.json` is in `.gitignore` (it's session-local state).
...
| `src/commands/install-plugin.ts` | CREATE | CLI install command | `src/commands/create-assignment.ts` L26-29 |
| `src/index.ts` | MODIFY | Register install-plugin command | `src/index.ts` L20-34 |
| `.gitignore` | MODIFY | Ignore .claude/syntaur.local.json | -- |
| `package.json` | MODIFY | Add plugin to files | -- |

## Dependencies & Risks
| Dependency/Risk | Impact | Mitigation |
|----------------|--------|------------|
| Chunk 4 lifecycle commands not yet merged | Skills invoke `syntaur assign`, `syntaur start`, `syntaur review`, `syntaur complete` which are being built concurrently | Skills can be built and tested with manual state setup. The skill instructions can include a "if command not found, the lifecycle engine may not be installed yet" note. The plugin directory structure and all markdown is independent of chunk 4 code. |
...
2. [ ] **Step 4.2:** Create `plugin/skills/grab-assignment/SKILL.md`
   ...
   - **Code:**
     ...
     ## Step 4: Read Assignment Context
     ...
     ```json
     {
       ...
       "missionDir": "~/.syntaur/missions/<mission-slug>",
       "assignmentDir": "~/.syntaur/missions/<mission-slug>/assignments/<assignment-slug>",
       "workspaceRoot": "<workspace.worktreePath or workspace.repository or null>",
...
| Scenario | Handling | User Message | Code |
|----------|----------|--------------|------|
| Mission not found | `ls` on mission dir fails | "Mission `<slug>` not found at `~/.syntaur/missions/<slug>/`. Run `syntaur create-mission` first." | N/A (skill body instruction) |
...
- [ ] Body includes all 6 steps: pre-flight, discover, find pending, claim, read context, create context file, report
...
2. [ ] **Step 5.2:** Create `plugin/skills/plan-assignment/SKILL.md`
   ...
   - **Code:**
     ...
     Read the existing `<assignmentDir>/plan.md` to see its current frontmatter structure. Preserve the YAML frontmatter fields (`assignmentSlug`, `title`, `created`, `updated`) and update the `updated` timestamp.
...
2. [ ] **Step 6.2:** Create `plugin/skills/complete-assignment/SKILL.md`
   ...
   - **Code:**
     ...
     If the user passed `--complete`, transition directly to `completed` instead of `review`.
     ...
     Append a new handoff entry to the markdown body. The entry should follow this format:
     ...
     - [x] <Criterion 1> -- <brief note on how it was met>
     - [x] <Criterion 2> -- <brief note>
     - [ ] <Unmet criterion> -- <why not met>

     ### Known Issues
     - <Any issues, caveats, or follow-up items>
     ...
     Also update the `updated` field in the handoff.md frontmatter to the current timestamp.
...
3. [ ] **Step 7.3:** Create `plugin/hooks/enforce-boundaries.sh`
   ...
   - **Code:**
     ...
     # Allow: files in mission resources/ directory
     if [[ "$FILE_PATH" == "$MISSION_DIR/resources/"* ]]; then
       allow_and_exit
     fi

     # Allow: files in mission memories/ directory
...
### Task 8: Create install-plugin CLI command

**File(s):** `src/commands/install-plugin.ts`, `src/index.ts`
**Action:** CREATE (install-plugin.ts), MODIFY (index.ts)
**Pattern Reference:** `/Users/brennen/syntaur/src/commands/create-assignment.ts:26-29`, `/Users/brennen/syntaur/src/index.ts:20-34`
**Estimated complexity:** Medium

#### Context
This is the only TypeScript file in the chunk. It creates a `syntaur install-plugin` CLI command that symlinks the `plugin/` directory from the repo into `~/.claude/plugins/syntaur/` so Claude Code can discover the plugin.
...
### Task 9: Update .gitignore and package.json

**File(s):** `.gitignore`, `package.json`
**Action:** MODIFY
**Pattern Reference:** Current file contents
**Estimated complexity:** Low

#### Context
The `.gitignore` needs to exclude the session-local context file. The `package.json` needs the `plugin` directory in its `files` array so the plugin ships with npm packages.
...
1. [ ] **Step 9.1:** Add `.claude/syntaur.local.json` to `.gitignore`
   ...
   - **Code (append to end of file):**
     ...
     ```
   - **Proof blocks:**
     - **PROOF:** Current `.gitignore` contents (5 lines).
       Source: `/Users/brennen/syntaur/.gitignore:1-5`
       Actual code:
       ```
       node_modules/
       dist/
...
2. [ ] **Step 9.2:** Add `plugin` to `files` array in `package.json`
   ...
   - **Code (change the files array):**
     ...
     ```
   - **Proof blocks:**
     - **PROOF:** Current `files` array.
       Source: `/Users/brennen/syntaur/package.json:10-13`
       Actual code:
       ```json
       "files": [
         "dist",
...
### Counts
- **Tasks:** 9
- **Total steps:** 20 (1.1-1.2, 2.1-2.3, 3.1-3.2, 4.1-4.2, 5.1-5.2, 6.1-6.2, 7.1-7.4, 8.1-8.2, 9.1-9.2)
- **Proof blocks:** 38
- **Files to create:** 11 (plugin.json, protocol-summary.md, file-ownership.md, 4 SKILL.md files, hooks.json, enforce-boundaries.sh, install-plugin.ts)
- **Files to modify:** 3 (index.ts, .gitignore, package.json)

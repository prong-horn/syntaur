...
## Revision 3 — 2026-04-23

**Reason:** Codex review #3 (`/tmp/codex-plan-review-3.md`) — small, tight follow-ups.

**What changed:**
- Task 5: Project-existence check now uses `project.md` (not directory) to match real discovery at `src/dashboard/api.ts:1141`. An empty `ghost/` directory with no `project.md` is NOT a valid project.
- Task 5: Explicit error handling for the pre-`writeChecklist` `mkdir` calls — swallow `EEXIST`; map `ENOENT` to the 404 "Project not found" response (don't let it bubble as 500).
- Task 5 & 13: `readConfig()` is async (`src/utils/config.ts:526`). All call sites must `await readConfig()` first. Updated.
- Task 13: Semantic tightening — count the number of mutually-exclusive flags first, error if >1, THEN resolve the no-flag default. Current wording put the error after the resolution branches.
- Task 13 & 15: CLI existence check uses `<projectsDir>/<slug>/project.md` (not directory). Added test case (j) for empty `ghost/` dir with no `project.md` still failing.
- Accuracy: `WsMessage.projectSlug` sits at `src/dashboard/types.ts:386` (the `WsMessage` interface itself starts at :384). No functional change, just the anchor.
...
| `src/__tests__/cli-todo-scope.test.ts` | CREATE | CLI scope resolution: no-flag default, ghost-project failure, combo rejection |

## Tasks

### 1. Path helper
- **File:** `src/utils/paths.ts` (MODIFY)
- **What:** Export `projectTodosDir(projectsDir: string, projectSlug: string): string` returning `resolve(projectsDir, projectSlug, 'todos')`. Signature takes the configured projects root explicitly — callers pass `config.defaultProjectDir` (dashboard/router, where the config is already read and injected) or `(await readConfig()).defaultProjectDir` (CLI, where `readConfig` is async per `src/utils/config.ts:526`). Do NOT re-export a zero-arg variant that falls back to the built-in `defaultProjectDir()` — this would silently diverge from the dashboard/backup root.
...
- **What:** Export `createProjectTodosRouter(projectsDir: string, broadcast: (msg: WsMessage)=>void)` returning `Router({ mergeParams: true })` so `:projectId` from the parent mount is accessible in handlers. **14 routes** (no all-projects aggregate — workspace `GET /api/todos` has no equivalent here):
  ...
  **Implementation rules:**
  ...
  - Then check project existence by probing `project.md` (matches real project-discovery pattern at `src/dashboard/api.ts:1141`): `const projectMd = resolve(projectsDir, req.params.projectId, 'project.md'); if (!(await fileExists(projectMd))) { res.status(404).json({ error: `Project "${req.params.projectId}" not found` }); return; }`. This rejects empty-directory "ghost" projects that have no `project.md`.
  - Resolve todos dir per-request via `projectTodosDir(projectsDir, req.params.projectId)`.
  - **TOCTOU mitigation on write paths (POST/PATCH/DELETE/POST /:id/*):** inside the `withLock('proj:<slug>')` critical section, re-probe `await fileExists(projectMd)` before any parser call. If `project.md` disappeared, return 404 and do NOT write. Then, before the first `writeChecklist` or `appendLogEntry`, create only the `todos/` subdirectory (`await mkdir(projectTodosDir(projectsDir, slug), { recursive: false })`) and `todos/archive/`. Error handling:
    - Catch `EEXIST` silently — directory already present.
    - Catch `ENOENT` (project parent was removed between the re-probe and `mkdir`) → return the same 404 response, NOT a 500. Don't let `ENOENT` bubble as an unhandled rejection.
    - Any other error → 500 via the existing router error middleware.
    This avoids relying on parser-side recursive `ensureDir` (`src/todos/parser.ts:264,279`) that would silently re-create a deleted project parent.
...
- **What:** Add `async function resolveScope({ project?, workspace?, global? }): Promise<{ kind: 'project'|'workspace', id: string }>` next to `resolveWorkspace` at :19-30. Semantics — **at most one** of the three flags may be present; no-flag is a valid case. Implement in this order:
  1. **Count first:** `const flagCount = [!!project, !!workspace, !!global].filter(Boolean).length;`. If `flagCount > 1` → `todoCommand.error('Use at most one of --project, --workspace, --global')` and return (unreachable, commander exits).
  2. **Project branch:** if `project` is set, validate with `isValidSlug(project)` (emit `todoCommand.error(`Invalid project slug "${project}"`)` if invalid). Then `const config = await readConfig();` (async — `src/utils/config.ts:526`). Then `const projectMd = resolve(config.defaultProjectDir, project, 'project.md'); if (!(await fileExists(projectMd))) { todoCommand.error(`Project "${project}" not found`); }`. Import `fileExists` from `../utils/fs.js`. Checking `project.md` (not the directory) matches dashboard project discovery at `src/dashboard/api.ts:1141` and prevents an empty `ghost/` directory from being treated as a valid project. Return `{ kind: 'project', id: project }`.
  3. **Workspace branch:** else if `workspace` is set → `{ kind: 'workspace', id: workspace }`.
  4. **Default:** else (no-flag OR `global: true`) → `{ kind: 'workspace', id: '_global' }`, preserving existing default at `src/commands/todo.ts:21`.
  - **TOCTOU note:** CLI is single-process; inline check is sufficient. CLI writes are not synchronized — matches existing workspace-todo CLI behavior (see Risks).

  Add `--project <slug>` option to each subcommand declaration: `add` (:39), `list` (:65), `start` (:116), `complete` (:147), `block` (:188), `unblock` (:228), `delete`, `edit`, `tag`, `log`, `archive`, `promote` (where it now means SOURCE scope; target project is `--to-project` from task 12). In each action body, `const scope = await resolveScope(options);` then resolve `todosPath`:
  - `kind==='project'` → `const config = await readConfig(); const todosPath = projectTodosDir(config.defaultProjectDir, scope.id);` and pass `scope.id` as the scope key (written to frontmatter `workspace:` field). (Action callbacks are already async-capable.)
  - `kind==='workspace'` → `todosPath = getTodosDir()` and pass `scope.id` (which will be `_global` or a workspace slug) as the scope key.
- **Pattern:** Reuse every existing parser call site unchanged; swap only the two inputs (todos dir and scope key).
- **Verify:** `npm run typecheck`; `npx syntaur todo add "foo" --project test-proj` round-trips; `npx syntaur todo add "foo" --project x --workspace y` errors with "Use at most one..."; `npx syntaur todo add "foo" --project ghost` (non-existent project, or existing dir without `project.md`) errors with "Project not found" and does NOT create `<projectsDir>/ghost/`; `npx syntaur todo add "foo"` (no flags) still writes to `getTodosDir()/_global.md`.
...
- **What:** CLI-level tests for scope resolution:
  ...
  - (h) `syntaur todo add "foo" --project ghost` (no such directory) exits non-zero, prints "Project not found", and does NOT create `<projectsDir>/ghost/` on disk (assert directory absence after the process exits).
  - (i) `syntaur todo add "foo" --project x --workspace y` exits non-zero with "Use at most one...".
  - (j) Pre-create an empty `<projectsDir>/ghost/` directory with NO `project.md` inside it. `syntaur todo add "foo" --project ghost` still exits non-zero with "Project not found" and does NOT create `<projectsDir>/ghost/todos/` (confirms the check reads `project.md`, not just dir existence).

...
- [ ] Plugin skills reference workspace in mission discovery guidance (both in-repo and Codex-side copies)
- [ ] All internal links preserve workspace prefix when navigating within a workspace
- [ ] Full typecheck passes (server and dashboard)
...
| `src/dashboard/api-write.ts` | Write API | No changes needed -- `renderMission()` workspace param is optional |
...
| `dashboard/src/components/SidebarNav.tsx` | Flat nav list | No changes needed -- reused as-is for global and utility nav zones |
...
10a. **Workspace-aware link propagation:** Update all hardcoded `/missions/...` links across ~15 pages to preserve workspace prefix via `useWorkspacePrefix()` hook -- Files: `MissionDetail.tsx`, `AssignmentDetail.tsx`, `MissionList.tsx`, `AssignmentsPage.tsx`, `AgentSessionsPage.tsx`, `ServersPage.tsx`, `CreateAssignment.tsx`, `EditMission.tsx`, `EditAssignment.tsx`, `EditAssignmentPlan.tsx`, `EditAssignmentScratchpad.tsx`, `AppendAssignmentHandoff.tsx`, `AppendAssignmentDecisionRecord.tsx`, `TopBar.tsx`

11. **Plugin skill updates:** Add workspace guidance to create-mission and grab-assignment skills (both in-repo and Codex-side copies) -- Files: `plugin/skills/create-mission/SKILL.md`, `plugin/skills/grab-assignment/SKILL.md`, `plugins/syntaur/skills/create-mission/SKILL.md`, `plugins/syntaur/skills/grab-assignment/SKILL.md`

12. **Example update:** Add workspace field to sample mission -- Files: `examples/sample-mission/mission.md`

13. **Verification and typecheck gate:** Full typecheck (server + dashboard), build, and smoke tests -- No file changes
...
| `src/dashboard/api-write.ts` | UNCHANGED | No changes needed -- `renderMission()` workspace param is optional | Template route at L168-176 |
...
| `dashboard/src/components/SidebarNav.tsx` | UNCHANGED | Reused as-is for global and utility zones; workspace items use inline `<Link>` elements | N/A |
...
| `plugins/syntaur/skills/create-mission/SKILL.md` | MODIFY | Mirror `--workspace` option (Codex-side copy) | N/A (documentation) |
| `plugins/syntaur/skills/grab-assignment/SKILL.md` | MODIFY | Mirror workspace awareness (Codex-side copy) | N/A (documentation) |
| `dashboard/src/pages/MissionDetail.tsx` | MODIFY | Prefix all internal links with workspace context | Task 10a `useWorkspacePrefix()` pattern |
| `dashboard/src/pages/AssignmentDetail.tsx` | MODIFY | Prefix all internal links with workspace context | Task 10a `useWorkspacePrefix()` pattern |
| `dashboard/src/pages/CreateAssignment.tsx` | MODIFY | Prefix post-create navigation and cancel link | Task 10a `useWorkspacePrefix()` pattern |
| `dashboard/src/pages/EditMission.tsx` | MODIFY | Prefix `redirectTo` with workspace context | Task 10a `useWorkspacePrefix()` pattern |
| `dashboard/src/pages/EditAssignment.tsx` | MODIFY | Prefix `redirectTo` with workspace context | Task 10a `useWorkspacePrefix()` pattern |
| `dashboard/src/pages/EditAssignmentPlan.tsx` | MODIFY | Prefix `redirectTo` with workspace context | Task 10a `useWorkspacePrefix()` pattern |
| `dashboard/src/pages/EditAssignmentScratchpad.tsx` | MODIFY | Prefix `redirectTo` with workspace context | Task 10a `useWorkspacePrefix()` pattern |
| `dashboard/src/pages/AppendAssignmentHandoff.tsx` | MODIFY | Prefix `redirectTo` with workspace context | Task 10a `useWorkspacePrefix()` pattern |
| `dashboard/src/pages/AppendAssignmentDecisionRecord.tsx` | MODIFY | Prefix `redirectTo` with workspace context | Task 10a `useWorkspacePrefix()` pattern |
| `dashboard/src/components/TopBar.tsx` | MODIFY | Prefix create assignment link with workspace context | Task 10a `useWorkspacePrefix()` pattern |
...
| Scenario | Handling | User Message | Code |
|----------|----------|--------------|------|
| No missions exist | `listMissionRecords()` returns `[]` | Returns `{ workspaces: [], hasUngrouped: false }` | Handled by empty set |
| All missions have `workspace: null` | No workspace names added to set | Returns `{ workspaces: [], hasUngrouped: true }` | Handled by the `if/else` check |
| missionsDir does not exist | `listMissionRecords()` returns `[]` | Returns `{ workspaces: [], hasUngrouped: false }` | Handled at line 539-541 |
...
- [ ] `GET /api/workspaces` returns `{ workspaces: string[], hasUngrouped: boolean }`
...
**File(s):** `dashboard/src/App.tsx`, `dashboard/src/components/Layout.tsx`, `dashboard/src/components/AppShell.tsx`
**Action:** MODIFY
**Pattern Reference:** `dashboard/src/App.tsx:29-53` (route definitions), `dashboard/src/components/Layout.tsx:1-14`
**Estimated complexity:** Medium
**IMPORTANT: Ordering dependency** — Step 7.2 passes `workspace` to `AppShell`. You MUST first add the `workspace` prop to `AppShellProps` (Task 9, Step 9.1) before this step, or the code won't compile. Implement Step 9.1 here before proceeding to Step 7.2.
...
3. [ ] **Step 10.3:** Add workspace filtering to `AgentSessionsPage.tsx`
   ...
   - **Code (add workspace filter in the `filteredSessions` useMemo, at line 68, before the `query` check):**
     ```typescript
         // When workspace filter is active but mission data hasn't loaded yet, show nothing to avoid flash of unfiltered content
   ...
   - **Proof blocks:**
     ...
     - **PROOF:** `AgentSession` has `missionSlug: string | null`.
       Source: `dashboard/src/types.ts:70`
...
4. [ ] **Step 10.4:** Add workspace filtering to `ServersPage.tsx`
   ...
   - **Code (filter sessions before rendering, replace `data.sessions` usage in the JSX with `filteredSessions`):**
     ...
     ```typescript
       ...
       const filteredSessions = (() => {
         ...
         if (!missionsData) return []; // Wait for mission data to load before showing anything
   ...
   - **Proof blocks:**
     ...
     - **PROOF:** `TrackedPane.assignment` has `{ mission: string; slug: string; title: string } | null`.
       Source: `dashboard/src/types.ts:25-29`
...
### Task 10a: Workspace-Aware Link Propagation
**File(s):** `dashboard/src/pages/MissionDetail.tsx`, `dashboard/src/pages/AssignmentDetail.tsx`, `dashboard/src/pages/MissionList.tsx`, `dashboard/src/pages/AssignmentsPage.tsx`, `dashboard/src/pages/AgentSessionsPage.tsx`, `dashboard/src/pages/ServersPage.tsx`, `dashboard/src/pages/CreateAssignment.tsx`, `dashboard/src/pages/EditMission.tsx`, `dashboard/src/pages/EditAssignment.tsx`, `dashboard/src/pages/EditAssignmentPlan.tsx`, `dashboard/src/pages/EditAssignmentScratchpad.tsx`, `dashboard/src/pages/AppendAssignmentHandoff.tsx`, `dashboard/src/pages/AppendAssignmentDecisionRecord.tsx`, `dashboard/src/components/TopBar.tsx`
**Action:** MODIFY
**Estimated complexity:** Medium-High (repetitive but broad)

#### Context
When navigating within a workspace context (`/w/:workspace/...`), all internal links must preserve the workspace prefix. Without this, clicking a mission link from `/w/syntaur/missions` would navigate to `/missions/<slug>` (global) instead of `/w/syntaur/missions/<slug>` (workspace-scoped), breaking the sidebar active state and losing workspace context.

The pattern is consistent: import `useParams`, extract `workspace`, and prefix all navigation links with `/w/${workspace}` when workspace is defined.

#### Steps

1. [ ] **Step 10a.1:** Create a shared `useWorkspacePrefix()` helper hook
   - **Location:** `dashboard/src/hooks/useMissions.ts` (append after `useWorkspaces()`)
   - **Action:** MODIFY (insert new function)
   - **What to do:** Add a tiny hook that returns the URL prefix for the current workspace context, to avoid repeating the same `useParams` + prefix logic in every page.
   - **Code:**
     ```typescript
     ...
     ```
   - **Verification:** Import and call in any page — returns `/w/syntaur` when on a workspace route, `''` when on a flat route.

2. [ ] **Step 10a.2:** Update `MissionList.tsx` — all mission links
   - **Location:** `dashboard/src/pages/MissionList.tsx:166,221,362`
   - **Action:** MODIFY
   - **What to do:** Import `useWorkspacePrefix`, call it in the component, and prefix all mission links.
   - **Code (add import):**
     ```typescript
     import { useMissions, useWorkspacePrefix } from '../hooks/useMissions';
     ```
   - **Code (add to component body):**
     ```typescript
     const wsPrefix = useWorkspacePrefix();
     ```
   - **Code (update links — 3 locations):**
     ```tsx
     // Line 166: mission card link
     to={`${wsPrefix}/missions/${mission.slug}`}
     // Line 221: mission table row link
     to={`${wsPrefix}/missions/${mission.slug}`}
     // Line 362: compact mission link
     to={`${wsPrefix}/missions/${mission.slug}`}
     ```
   - **Note:** The Create Mission link at line 156 was already updated in Task 10 Step 10.1.

3. [ ] **Step 10a.3:** Update `MissionDetail.tsx` — all links
   - **Location:** `dashboard/src/pages/MissionDetail.tsx:33,104,108,191,219,331,335,379`
   - **Action:** MODIFY
   - **What to do:** Import `useWorkspacePrefix`, prefix all internal links.
   - **Code (add import and call):**
     ```typescript
     import { useWorkspacePrefix } from '../hooks/useMissions';
     // In component:
     const wsPrefix = useWorkspacePrefix();
     ```
   - **Code (update links — 8 locations):**
     ```tsx
     // Line 33: assignment card route
     const route = `${wsPrefix}/missions/${mission.slug}/assignments/${assignment.slug}`;
     // Line 104: edit mission
     to={`${wsPrefix}/missions/${mission.slug}/edit`}
     // Line 108: create assignment
     to={`${wsPrefix}/missions/${mission.slug}/create/assignment`}
     // Line 191: create assignment (empty state)
     to={`${wsPrefix}/missions/${mission.slug}/create/assignment`}
     // Line 219: assignment link
     to={`${wsPrefix}/missions/${mission.slug}/assignments/${assignment.slug}`}
     // Line 331: edit mission (sidebar)
     to={`${wsPrefix}/missions/${mission.slug}/edit`}
     // Line 335: create assignment (sidebar)
     to={`${wsPrefix}/missions/${mission.slug}/create/assignment`}
     // Line 379: AssignmentCard link (passed as prop or inside component)
     to={`${wsPrefix}/missions/${missionSlug}/assignments/${assignment.slug}`}
     ```

4. [ ] **Step 10a.4:** Update `AssignmentDetail.tsx` — all links
   - **Location:** `dashboard/src/pages/AssignmentDetail.tsx:163,166,317,344,469,473,477,481,485`
   - **Action:** MODIFY
   - **What to do:** Import `useWorkspacePrefix`, prefix all internal links.
   - **Code (update links — 9 locations):**
     ```tsx
     // Line 163: back to mission
     to={`${wsPrefix}/missions/${slug}`}
     // Line 166: edit assignment
     to={`${wsPrefix}/missions/${slug}/assignments/${aslug}/edit`}
     // Line 317: edit plan
     to={`${wsPrefix}/missions/${slug}/assignments/${aslug}/plan/edit`}
     // Line 344: edit scratchpad
     to={`${wsPrefix}/missions/${slug}/assignments/${aslug}/scratchpad/edit`}
     // Line 469: edit assignment (sidebar)
     to={`${wsPrefix}/missions/${slug}/assignments/${aslug}/edit`}
     // Line 473: edit plan (sidebar)
     to={`${wsPrefix}/missions/${slug}/assignments/${aslug}/plan/edit`}
     // Line 477: edit scratchpad (sidebar)
     to={`${wsPrefix}/missions/${slug}/assignments/${aslug}/scratchpad/edit`}
     // Line 481: append handoff (sidebar)
     to={`${wsPrefix}/missions/${slug}/assignments/${aslug}/handoff/edit`}
     // Line 485: append decision (sidebar)
     to={`${wsPrefix}/missions/${slug}/assignments/${aslug}/decision-record/edit`}
     ```

5. [ ] **Step 10a.5:** Update `AssignmentsPage.tsx` — assignment links
   - **Location:** `dashboard/src/pages/AssignmentsPage.tsx:490,520,723`
   - **Action:** MODIFY
   - **What to do:** Import `useWorkspacePrefix`, prefix all internal links.
   - **Code (update links — 3 locations):**
     ```tsx
     // Line 490: "View all missions" link
     to={`${wsPrefix}/missions`}
     // Line 520: assignment card link
     to={`${wsPrefix}/missions/${assignment.missionSlug}/assignments/${assignment.slug}`}
     // Line 723: assignment table row link
     to={`${wsPrefix}/missions/${assignment.missionSlug}/assignments/${assignment.slug}`}
     ```

6. [ ] **Step 10a.6:** Update `AgentSessionsPage.tsx` — session links
   - **Location:** `dashboard/src/pages/AgentSessionsPage.tsx:319,329`
   - **Action:** MODIFY
   - **What to do:** Import `useWorkspacePrefix`, prefix mission/assignment links.
   - **Code (update links — 2 locations):**
     ```tsx
     // Line 319: mission link
     to={`${wsPrefix}/missions/${session.missionSlug}`}
     // Line 329: assignment link
     to={`${wsPrefix}/missions/${session.missionSlug}/assignments/${session.assignmentSlug}`}
     ```

7. [ ] **Step 10a.7:** Update `ServersPage.tsx` — pane assignment link
   - **Location:** `dashboard/src/pages/ServersPage.tsx:260`
   - **Action:** MODIFY
   - **Code:**
     ```tsx
...
8. [ ] **Step 10a.8:** Update `CreateAssignment.tsx` — post-create navigation
   - **Location:** `dashboard/src/pages/CreateAssignment.tsx:51,79`
   - **Action:** MODIFY
   - **Code:**
     ```tsx
     // Line 51: navigate after creation
     navigate(`${wsPrefix}/missions/${slug}/assignments/${payload.slug}`);
     // Line 79: cancel navigation
     onCancel={() => navigate(slug ? `${wsPrefix}/missions/${slug}` : `${wsPrefix}/missions`)}
     ```

9. [ ] **Step 10a.9:** Update edit/append pages — `redirectTo` links
   - **Files:** `EditMission.tsx:12`, `EditAssignment.tsx:13`, `EditAssignmentPlan.tsx:13`, `EditAssignmentScratchpad.tsx:13`, `AppendAssignmentHandoff.tsx:13`, `AppendAssignmentDecisionRecord.tsx:13`
   - **Action:** MODIFY each file
   - **What to do:** These pages use a `redirectTo` prop on their editor component. Import `useWorkspacePrefix` and prefix the `redirectTo` value. The pattern is identical across all 6 files.
   - **Code example (EditMission.tsx):**
     ```tsx
     import { useWorkspacePrefix } from '../hooks/useMissions';
     // In component:
     const wsPrefix = useWorkspacePrefix();
   ...
   - **Apply the same pattern to:**
     - `EditAssignment.tsx:13` → `redirectTo={``${wsPrefix}/missions/${missionSlug}/assignments/${assignmentSlug}``}`
     - `EditAssignmentPlan.tsx:13` → `redirectTo={``${wsPrefix}/missions/${missionSlug}/assignments/${assignmentSlug}?tab=plan``}`
     - `EditAssignmentScratchpad.tsx:13` → `redirectTo={``${wsPrefix}/missions/${missionSlug}/assignments/${assignmentSlug}?tab=scratchpad``}`
     - `AppendAssignmentHandoff.tsx:13` → `redirectTo={``${wsPrefix}/missions/${missionSlug}/assignments/${assignmentSlug}?tab=handoff``}`
     - `AppendAssignmentDecisionRecord.tsx:13` → `redirectTo={``${wsPrefix}/missions/${missionSlug}/assignments/${assignmentSlug}?tab=decisions``}`

10. [ ] **Step 10a.10:** Update `TopBar.tsx` — create assignment link
    - **Location:** `dashboard/src/components/TopBar.tsx:57`
    - **Action:** MODIFY
    - **What to do:** Import `useWorkspacePrefix` and prefix the create assignment link.
    - **Code:**
      ```tsx
      to={`${wsPrefix}/missions/${missionSlug}/create/assignment`}
      ```

#### Error Handling
| Scenario | Handling | User Message | Code |
|----------|----------|--------------|------|
| `useWorkspacePrefix()` on flat route | Returns `''` — links stay as `/missions/...` | N/A | Empty string prefix is a no-op |
| Navigating between workspace and global views | Links are relative to current context | N/A | `wsPrefix` is always correct for current URL |

#### Task Completion Criteria
- [ ] All internal links preserve workspace prefix when in workspace context
- [ ] Navigation from workspace-scoped pages stays within workspace
- [ ] Flat routes (no `/w/` prefix) produce unchanged links
- [ ] `useWorkspacePrefix()` hook exists and is used across all pages
- [ ] Code compiles without errors

---

### Task 11: Plugin Skill Updates
**File(s):** `plugin/skills/create-mission/SKILL.md`, `plugin/skills/grab-assignment/SKILL.md`, `plugins/syntaur/skills/create-mission/SKILL.md`, `plugins/syntaur/skills/grab-assignment/SKILL.md`
**Action:** MODIFY
**Estimated complexity:** Low
**Note:** There are TWO copies of skill files — `plugin/skills/` (the in-repo plugin) and `plugins/syntaur/skills/` (the installed/Codex-side plugin). Both must be updated to stay in sync.
...
3. [ ] **Step 11.3:** Mirror changes to Codex-side `plugins/syntaur/skills/create-mission/SKILL.md`
   - **Location:** `plugins/syntaur/skills/create-mission/SKILL.md`
   - **Action:** MODIFY
   - **What to do:** Apply the exact same changes from Step 11.1 to the Codex-side copy: add `--workspace` to argument-hint, parsing section, and CLI example.

4. [ ] **Step 11.4:** Mirror changes to Codex-side `plugins/syntaur/skills/grab-assignment/SKILL.md`
   - **Location:** `plugins/syntaur/skills/grab-assignment/SKILL.md`
   - **Action:** MODIFY
   - **What to do:** Apply the exact same workspace awareness note from Step 11.2 to the Codex-side copy.
...
- [ ] `plugins/syntaur/skills/create-mission/SKILL.md` mirrors the same changes
- [ ] `plugins/syntaur/skills/grab-assignment/SKILL.md` mirrors the same changes
...
### Task 13: Verification and Typecheck Gate
**File(s):** N/A (verification only)
**Action:** VERIFY
**Estimated complexity:** Low

#### Context
After all implementation tasks, run a full typecheck and manual smoke test to catch any issues introduced by the workspace changes. This is the final gate before the feature is considered complete.

#### Steps

1. [ ] **Step 13.1:** Run full server-side typecheck
   - **Command:** `npx tsc --noEmit`
   - **Expected:** Zero errors. All new `workspace` fields must align across parser, types, and API.

2. [ ] **Step 13.2:** Run full dashboard typecheck
   - **Command:** `cd dashboard && npx tsc --noEmit`
   - **Expected:** Zero errors. All `useWorkspacePrefix()` calls, `useParams` extractions, and workspace prop additions must be type-safe.

3. [ ] **Step 13.3:** Run dashboard build
   - **Command:** `cd dashboard && npm run build`
   - **Expected:** Successful build with no errors or warnings related to workspace changes.

4. [ ] **Step 13.4:** Smoke test — global routes still work
   - Navigate to `/`, `/attention`, `/playbooks`, `/help`, `/settings`
   - **Expected:** All pages render without errors. Sidebar shows three zones.

5. [ ] **Step 13.5:** Smoke test — workspace routes work
   - Navigate to `/w/_ungrouped/missions`, `/w/_ungrouped/assignments`
   - **Expected:** Shows missions/assignments that have no workspace set.

6. [ ] **Step 13.6:** Smoke test — link propagation
   - From a workspace-scoped mission list, click a mission, then an assignment, then edit
   - **Expected:** URL stays within `/w/:workspace/...` prefix throughout navigation. Sidebar active state is correct.

7. [ ] **Step 13.7:** Smoke test — mobile sidebar
   - Open mobile nav, click a workspace item
   - **Expected:** Mobile overlay closes after click (onNavigate fires).
...
- [ ] Workspace routes filter correctly
- [ ] Link propagation preserves workspace context
...
- **Total tasks:** 14 (12 original + Task 10a link propagation + Task 13 verification)
- **Total steps:** 49 (32 original + 10 link propagation steps + 7 verification steps)
- **Total proof blocks:** 58 (original, updated with correct line references)
...
The following issues from the Codex gpt-5.4 review and the sun-tzu Phase 4 review were addressed:

1. **FIXED: Completeness — hardcoded links.** Added Task 10a with 10 steps covering all ~15 pages with hardcoded `/missions/...` links. Introduced a `useWorkspacePrefix()` helper hook to avoid repetitive `useParams` boilerplate.
2. **FIXED: Completeness — Codex-side plugin skills.** Expanded Task 11 with Steps 11.3 and 11.4 to mirror changes to `plugins/syntaur/skills/` copies.
3. **FIXED: Accuracy — bad line references.** Corrected `dashboard/src/types.ts:373-374` → `:70` and `:342-347` → `:25-29`.
4. **FIXED: Accuracy — api-write.ts contradiction.** Changed both file tables from MODIFY to UNCHANGED with explanation.
5. **FIXED: Accuracy — SidebarNav.tsx.** Changed from MODIFY to UNCHANGED in both file tables.
6. **FIXED: Ordering — Task 7/9 dependency.** Added ordering dependency note to Task 7 header requiring Step 9.1 be implemented first.
7. **FIXED: Gaps — ungrouped always shown.** Updated `listWorkspaces()` to return `hasUngrouped: boolean`; sidebar conditionally renders `_ungrouped` section.
8. **FIXED: Gaps — mobile sidebar loses onNavigate.** Added `onNavigate` prop to `ShellSidebar`; propagated to all `<Link>` elements and `<SidebarNav>` calls.
9. **FIXED: Gaps — loading flash.** Added guards in AgentSessionsPage and ServersPage to return empty results when workspace filter is active but mission data hasn't loaded yet.
10. **FIXED: Gaps — no verification task.** Added Task 13 with 7 verification steps (typecheck, build, smoke tests).

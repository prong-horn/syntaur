...
- **Inherited user configuration (02, 05).** Both adapters inherit the local user's setup: claude-agent-acp picks up `~/.claude/settings.json` through `settingSources` (hooks, MCP servers, allow rules), codex-acp picks up `~/.codex/config.toml` (plugins, MCP servers, model, effort). Both started at this machine's `xhigh` (`effort` / `reasoning_effort`), and given a vague "search tool" prompt codex reached for its configured MCP tools (`mcp.codex_apps.github.*`, `mcp.node_repl.js`) before local shell. Every one of these is settable at session creation — `_meta.claudeCode.options.settingSources` and `session/new`'s `mcpServers` for the environment, `session/set_config_option` for model and effort, `session/set_mode` for the mode — so Syntaur *can* pin them per session whenever it wants to. Phase 2 chooses not to; see the session profile below.
...
- §5 costs a chat turn as the conversation. The dominant cost is the session's inherited environment — ~37 k tokens on claude, ~23 k on codex, before the first word, re-sent every turn, plus a forked copy of the user's MCP servers per open chat. Phase 2 accepts that cost deliberately rather than designing it away; the session profile below is how it stays changeable.
...
- `claude-agent-acp` inherits `settingSources` (user hooks, MCP servers, allow rules) and `codex-acp` inherits `~/.codex/config.toml` (plugins, MCP servers, model, effort). Phase 2 keeps that inheritance on purpose but routes it through one resolver so it can be pinned later without re-plumbing — see §5.11's session-profile decision.
...
- **What environment does a chat session load?** *(decided 2026-09-02: inherit the machine's defaults, behind one resolver.)* A chat session gets exactly what the same agent would get if it were launched by hand in that worktree — the user's `~/.claude/settings.json` / `~/.codex/config.toml`, their MCP servers, their model and effort — because an assignment chat that behaves differently from the terminal agent working the same assignment is a surprise, not a feature, and pinning a leaner environment is a tuning exercise with no phase-2 requirement driving it. The measured price is ~37 k tokens of standing context per claude session (~23 k codex), re-sent every turn, plus a forked copy of every configured MCP server per live session (§5.9b). What phase 2 *must* build is the seam, not the policy: the session broker resolves a single **session profile** — `settingSources`, `mcpServers`, `model`, `effort`, `mode` — in one place before `session/new`, with every field defaulting to "inherit". Shipping it with only the default costs a few lines; retrofitting the parameter later means threading it through the broker, the persisted session row and the UI. Two consequences worth pricing in now: sessions spawn lazily on the first message rather than when a chat is opened, and an idle session is torn down (a claude adapter group is ~620 MB with MCP servers forked in, so three idle chats is ~1.9 GB) and re-attached through Decision 7's `session/resume`.
...
- 2026-09-02 — Phase-2 session environment decided (§5.11): a chat session inherits the machine's defaults — the user's settings, MCP servers, model and effort — so it behaves like the terminal agent on the same worktree. The requirement is the seam, not the policy: the broker resolves one session profile (`settingSources`, `mcpServers`, `model`, `effort`, `mode`) before `session/new` with every field defaulting to inherit, plus lazy spawn and idle teardown, so pinning a leaner environment later needs no re-plumbing.

...
### 5.9b Spike results (2026-09-01)

Phase 1 ran on 2026-09-01 against both adapters — `claude-agent-acp` 0.70.0 (Claude Code 2.1.258, `claude.ai` login) and `codex-acp` 1.7.0 (codex-cli 0.149.1, ChatGPT login) — driven by an `@agentclientprotocol/sdk` 1.4.0 client on Node 22.22.1, 18 scenarios per adapter, each operating on a disposable clone of the Syntaur repo at commit `2b3e02b4844b`. Evidence: `scripts/spike/acp/RESULTS.md` (matrix, measured numbers, findings by area), the direction-tagged NDJSON transcripts under `src/__tests__/fixtures/acp/` with their `manifest.json`, and the assignment's `decision-record.md`. Every row is PASS or observed on both adapters (17 is claude-only by design), and 7, 12 and 14 pass on both — the condition §5.9a set for "nothing in §5 needs to change". §5 still needs the changes below anyway, because 05, 07, 09, 10 and 13 contradict what this document assumed about tool-call shape, permission coverage, thinking signals, config notifications and mid-turn prompts.
...
| 13 | Second prompt during a turn; steering | PASS | PASS |
...
**The coalescing key is `messageId`, not the turn** (Decision 5). Row 04: every real chunk carries one — claude 12 of 12, codex 126 of 127 — and it changes when a new message starts inside a turn. Row 13 on codex shows the queued `QUEUED` reply arriving under a second `messageId` within the same turn, which a turn-keyed renderer would glue onto the preceding essay. The one chunk without a `messageId` was codex-acp forwarding codex's startup warning ahead of the first reply, on every fresh session: a chunk with no `messageId` is an adapter notice and must render as a system row, never as agent prose. The normalizer keeps a `messageId → bubble` map per session, and the `user_message_chunk`/`agent_message_chunk` replayed by `session/load` (row 14) coalesce identically.

**Syntaur serializes prompts client-side and does not steer** (Decision 6). Neither adapter behaves well enough to lean on. Row 13, claude: a second `session/prompt` mid-turn is queued natively (A ends `end_turn` at 32.8 s, B answers `QUEUED` at 33.1 s), but `_session/steering` returns `{outcome: "injected"}` and streams `STEERED` while the original `session/prompt` never resolves (90 s timeout). Row 13, codex: a second `session/prompt` is merged into the running turn — the essay completes, `QUEUED` follows under a new `messageId`, only the second request is answered and the first is orphaned (240 s timeout); steering works there but starts a new turn when sent while idle. So Syntaur holds at most one in-flight `session/prompt` per session, queues further user messages itself (rendered as "queued", withdrawable) and sends the next only after the current response resolves. Interruption is `session/cancel`, which resolves the prompt `cancelled` in 25–29 ms on claude and 5–9 ms on codex (rows 12 and 07). This supersedes §5.5's "default to queue and offer interrupt & steer".

**`session/load` after a restart, `session/resume` when the transcript is already held** (Decision 7). Row 14, both adapters: `load` replays the history as `user_message_chunk`/`agent_message_chunk` before its response, `resume` replays nothing, and both recall the pre-restart code word. Syntaur calls `load` when it holds no transcript for the session (fresh dashboard process, first attach) so the chat rebuilds from the replay, and `resume` when it already holds one (adapter-only restart) so the replay is not double-rendered. The replay path is the same normalizer as live streaming. `mcpServers: []` goes on both — required on load, optional on resume in SDK 1.4.
...
- **Thoughts (09).** claude-agent-acp 0.70 emitted no `agent_thought_chunk` in any scenario; codex does emit them on longer turns (4 during `/context`, 2 in 13) but produced none on the arithmetic prompt at `high`. At the inherited `xhigh` effort claude thinks silently for more than 45 s on the row-12 essay prompt, with no update of any kind between `usage_update` and the first text chunk (at `effort=low` it streams at ~4 s). There is no thinking signal to render on claude-agent-acp, so a turn can sit visibly silent for that whole stretch.
...
- §5.9a step 12's "cancel at 5 s" assumed a chunk within 5 s. At the inherited `xhigh` effort claude produces nothing for more than 45 s, so any cancel-latency measure must wait for the first chunk.
- §5.9a step 13's "queue vs steer" is not a single choice. claude queues but its steering leaves the prompt unresolved; codex steers but merges a second prompt into the running turn and orphans the first request. Syntaur serializes prompts client-side on both and relies on steering on neither.

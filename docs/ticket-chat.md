...
4. Everything the agent streams back is appended to
   `<ticket>/chat/events.jsonl` and rendered.
...
---
You are the planner for this ticket. Read ticket.md, produce a plan, and
...
writes nothing to any ticket log.
...
turn moves on, and a question is filed in the ticket's comments so it shows
up in your Inbox. Auto-approved requests never wait.

## Where the data lives

- `<ticket>/chat/events.jsonl` — the append-only source of truth: every ACP
  frame and every Syntaur-side event, in order. Routing rows (the human's
  messages, hand-offs, routing notices) are written under one per-ticket
  scope key rather than under any agent's, because they belong to the room.
- `<ticket>/chat/participants.json` — who is attached, which one is the
  default, and the hop budget.
- `<ticket>/chat/attachments/<uuid>__<name>.<ext>` — image files uploaded
  from the composer, referenced from `user.message` events by id (bytes are never
  embedded in `events.jsonl`).
- `<ticket>/progress.md` — chat-written entries end with `Chat turn \`<id>\``.
  Filed decisions and progress entries end with `_Filed from chat (…)._`. Standalone
  tickets file to `~/.syntaur/tickets/<uuid>/` rather than under a project.
...
`engagement` row. That is what puts per-turn cost on the ticket's usage rail.
...
The attached set lives in `<ticket>/chat/participants.json`:
...
`GET /api/tickets/:id/chat/session`. Before an agent's first session opens,

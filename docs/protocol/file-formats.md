...
Zero or more implementation plan files per assignment. Plans are **not scaffolded** — they are created on demand by `/plan-assignment`.

**Filename versioning:** The first plan for an assignment is `plan.md`. Subsequent plans use `plan-v2.md`, `plan-v3.md`, etc. — the smallest unused `plan-v<N>.md` where `N >= 2`. When requirements shift, create a new versioned plan file instead of rewriting the old one.
